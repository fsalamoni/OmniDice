...
const buffers: Record<WeightClass, Record<PhysicalMaterial, AudioBuffer[]>> = {
  HEAVY: { DICE: [], LEATHER: [], WOOD: [], SHAKE: [] },
  LIGHT: { DICE: [], LEATHER: [], WOOD: [], SHAKE: [] },
  MEDIUM: { DICE: [], LEATHER: [], WOOD: [], SHAKE: [] },
};

const indices: Record<WeightClass, Record<PhysicalMaterial, number>> = {
  HEAVY: { DICE: 0, LEATHER: 0, WOOD: 0, SHAKE: 0 },
  LIGHT: { DICE: 0, LEATHER: 0, WOOD: 0, SHAKE: 0 },
  MEDIUM: { DICE: 0, LEATHER: 0, WOOD: 0, SHAKE: 0 },
};
...
export function getRandomBuffer(
  ...
  const b = getAudioBuffers(weightClass, physicalMaterial);
  if (b.length === 0) return null;
  return b[Math.floor(Math.random() * b.length)];
}

/**
 * Get an audio buffer ensuring that every consecutive call returns the next buffer
 */
export function getNextBuffer(
  ...
  const buffer = b[index];
  indices[weightClass][physicalMaterial] = (index + 1) % b.length;
  return buffer;
}
